function theme_showcase --description 'Create a showcase of the current color theme.'
...
"
  ...
  if not functions -q __theme_showcase_prompt
    function __theme_showcase_prompt -a exit_status jobs text
      ...
      set -l rprompt
      set -l job_ids
      set -l job_name "vi theme_showcase_foo_bar_baz"
      set -l job_template $job_name"_%s_%s >/dev/null 2>&1 &"
      ...
      # Now remove the jobs
      set job_pids (jobs | grep $job_name | cut -f 2)
      for job_pid in $job_pids
        kill -9 $job_pid 2>&1 >/dev/null
      end
    end
  end # }}}
  ...
  for arg in $args
    ...
    if test "$mode" = 'short'
      ...
      __theme_showcase_prompt 0 0 (
        ...
        __theme_showcase_print normal " "
        eval "set_color $fish_color_param"
        eval "set_color $fish_color_valid_path"
      ...
      eval "set_color $fish_color_search_match $fish_pager_color_completion"
      printf "%-5s" "$tags[$i]"
      ...
      while test $tagc -le (count $tags)
        ...
        eval "set_color $fish_pager_color_completion"
        printf "%-5s" "$tags[$i]"
        __theme_showcase_print normal "  "
