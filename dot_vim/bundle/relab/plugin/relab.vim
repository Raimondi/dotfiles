...
if exists('g:relab_debug')
  autocmd RELab BufWritePost relab.vim,*/relab/*.vim source <afile>
  "      \ | call relab#tests#run()
  "autocmd BufWritePost */autoload/relab/id_key.txt
  "      \ | execute printf('source %s/parser.vim', expand('<afile>:p:h'))
  "      \ | call relab#tests#run()
  command! -nargs=+ RELabID echo map(relab#parser#new({})(<q-args>).elements, 'v:val.id')
  command! -nargs=+ RELabLiteral echo map(relab#parser#new({})(<q-args>).elements, 'v:val.literal')
  command! -nargs=+ RELabMagic echo map(relab#parser#new({})(<q-args>).elements, 'v:val.magic')
endif
