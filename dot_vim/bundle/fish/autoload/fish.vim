...
function! s:IsSwitch(lnum)
    ...
    let l:lnum = a:lnum
    let l:in_block = 0
    let l:stop_pat = '\v^\s*%(if|else|while|for|begin|function)>'
    let l:block_start_pat = '\v^\s*%(if|while|for|switch|begin|function)>'
    while l:lnum > 0
        let l:lnum = prevnonblank(l:lnum - 1)
...
function! fish#Format()
    if mode() =~# '\v^%(i|R)$' || !executable('fish_indent')
        return 1
    else
...
function! fish#Fold(...)
    let l:line = getline(get(a:, 1, v:lnum))
